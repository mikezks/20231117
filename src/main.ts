import { APP_INITIALIZER, EnvironmentProviders, Injectable, enableProdMode, importProvidersFrom, inject, makeEnvironmentProviders } from '@angular/core';
...
export function provideInitConfig(): EnvironmentProviders {
  return makeEnvironmentProviders([{
    ...
    multi: true,
    useFactory: (c = inject(InitConfigService)) => () =>
      fetch('./assets/config.json')
        ...
        .then(value => new Promise(resolve => setTimeout(() => resolve(value), 5_000)))
        .then(value => console.log('APP_INITIALIZER', value))
  }]);
}
...
export function MSALInstanceFactory(configService: InitConfigService): IPublicClientApplication {
    console.log('MSAL_INSTANCE', configService.config.value);
